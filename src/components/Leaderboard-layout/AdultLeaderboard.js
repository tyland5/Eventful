...
function Leaderboard () {
    ...
    const displayLeaderboard = async () => {
        await axios.post("https://www-student.cse.buffalo.edu/CSE442-542/2023-Spring/cse-442b/leaderboard-data.php", {category: "adult"}).then((val) =>{
    ...
    const SideNav = () => {
        return(
            <ul className='sideNav'>
                ...
                <Link to="/food-leaderboard" className="routing-link"><li><a href = '#food'><p className='category-text'>Food</p></a></li></Link>
                <Link to="/adult-leaderboard" className="routing-link"><li><a class='active' href = '#adult'><p className='category-text'>Adult</p></a></li></Link>
            </ul>
        )
    }
        ...
        return (
            <div className='leader'>
                ...
                {createLeaderboard.map((value, idx) => {
                    return (
                        <LeaderboardRow username={createLeaderboard[idx].user_id} rank={idx+1} rowColor={RowColor(idx)} badge={BadgeType(createLeaderboard[idx].adult)} eventsAttended={createLeaderboard[idx].adult}/>
                    )
                })}
            </div>
        )
