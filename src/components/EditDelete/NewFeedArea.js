...
const NewFeedArea = ({showFilterButton, query}) => {
  ...
  const handleButtonClick = () => {
    // Here, we're updating the state of NewFeedArea, which will trigger a re-render.
  ...
  const[error, setError] = useState("");
    ...
    useEffect(() => {
      ...
      let siteURL = "https://www-student.cse.buffalo.edu/CSE442-542/2023-Spring/cse-442b/loadUserEvent.php"
      let localURL= "http://localhost/loadUserEvent.php"
  ...
  return (
    ...
    {post.map((value, idx) => {
          return (
            <>
            <NewFeedPost post_id = {post[idx].post_id} pfp = {dummy_pfp} posterName = {post[idx].poster} title = {post[idx].title} eventID = {post[idx].post_id} 
        thumbnail= {`https://www-student.cse.buffalo.edu/CSE442-542/2023-Spring/cse-442b/uploads/${post[idx].thumbnail}`} numBookmarked = "400" eventTag= {post[idx].type} allowClickEvent={allowClickEvent} onButtonClick={handleButtonClick} />
        </>
           )
          })}
    </div>
