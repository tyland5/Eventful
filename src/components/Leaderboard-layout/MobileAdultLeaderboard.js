    ...
    const MobileBadgeNav = () => {
        return(
            <div className='mobileBadgeNav'>
                ...
                <Link to="/mobile-adult-leaderboard" className='routing-link'><a className="active" href = '#adult' style={{color: 'white'}}>Adult</a></Link>
            </div>
        )
    }
        ...
        return (
            <div className='leader'>
                ...
                {createLeaderboard.map((value, idx) => {
                    return (
                        <LeaderboardMobile username={createLeaderboard[idx].user_id} rank={idx+1} rowColor={RowColor(idx)} badge={BadgeType(createLeaderboard[idx].adult)} eventsAttended={createLeaderboard[idx].adult}/>
                    )
                })}
            </div>
        )
